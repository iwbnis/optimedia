...
class Response
{
    static $STATUS_SUCCESS = ['OK' => 200, 'CREATED' => 201, 'ACCEPTED' => 202, 'NON_AUTHORITATIVE_INFORMATION' => 203, 'NO_CONTENT' => 204, 'RESET_CONTENT' => 205, 'PARTIAL_CONTENT' => 206, 'MULTI_STATUS' => 207, 'ALREADY_REPORTED' => 208, 'IM_USED' => 226];
    const STATUS_OK = 200;
    ...
    private $root_error;
    public function __construct(\XeroPHP\Remote\Request $request, $response_body, $status, $headers)
    {
    ...
    public function parseBody()
    {
        ...
        $this->root_error = [];
        if (!isset($this->headers[\XeroPHP\Remote\Request::HEADER_CONTENT_TYPE])) {
            //Nothing to parse
            return;
        }
        //Iterate in priority order
        foreach ($this->headers[\XeroPHP\Remote\Request::HEADER_CONTENT_TYPE] as $ct) {
            list($content_type) = \explode(';', $ct);
            switch ($content_type) {
                case \XeroPHP\Remote\Request::CONTENT_TYPE_XML:
                    $this->parseXML();
                    break;
                case \XeroPHP\Remote\Request::CONTENT_TYPE_JSON:
                    $this->parseJSON();
                    break;
                case \XeroPHP\Remote\Request::CONTENT_TYPE_HTML:
                    $this->parseHTML();
                    break;
                default:
                ...
                case 'ValidationErrors':
                    if (\is_array($value)) {
                        foreach ($value as $error) {
                            $this->element_errors[$element_index] = \trim($error['Message'], '.');
                        }
                    }
                    break;
                case 'Warnings':
                    if (\is_array($value)) {
                        foreach ($value as $warning) {
                            $this->element_warnings[$element_index] = \trim($warning['Message'], '.');
                        }
                    }
                    break;
                default:
    ...
    {
        $json = \json_decode($this->response_body, \true);
