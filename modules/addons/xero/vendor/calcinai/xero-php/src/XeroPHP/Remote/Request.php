...
use EdgeHosting\GuzzleHttp\Psr7\Request as PsrRequest;
use EdgeHosting\GuzzleHttp\Psr7\Uri;
...
class Request
{
     ...
     */
    public function __construct(Application $app, \XeroPHP\Remote\URL $url, $method = self::METHOD_GET)
    {
        $this->app = $app;
        $this->url = $url;
        $this->headers = [];
        ...
        switch ($method) {
            ...
            case self::METHOD_DELETE:
                ...
                break;
            default:
                throw new \XeroPHP\Remote\Exception("Invalid request method [{$method}]");
        }
    ...
    public function send()
    {
        ...
        } catch (\EdgeHosting\GuzzleHttp\Exception\BadResponseException $e) {
            $guzzleResponse = $e->getResponse();
        }
        $this->response = new \XeroPHP\Remote\Response($this, $guzzleResponse->getBody()->getContents(), $guzzleResponse->getStatusCode(), $guzzleResponse->getHeaders());
        $this->response->parse();
