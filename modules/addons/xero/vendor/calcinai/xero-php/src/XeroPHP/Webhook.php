...
class Webhook
{
    ...
    public function __construct($application, $payload, $event = null)
    {
        ...
        $this->payload = \json_decode($this->rawPayload, \true);
        if ($event === null) {
            $event = \XeroPHP\Webhook\Event::class;
        }
        $this->webhookEvent = $event;
        // bail if json_decode fails
        if ($this->payload === null) {
            throw new \XeroPHP\Exception('The webhook payload could not be decoded: ' . \json_last_error_msg());
        }
        // bail if we don't have all the fields we are expecting
        if (!isset($this->payload['events']) || !isset($this->payload['firstEventSequence']) || !isset($this->payload['lastEventSequence'])) {
            throw new \XeroPHP\Exception('The webhook payload was malformed');
        }
    }
    /**
    ...
    {
        return \base64_encode(\hash_hmac('sha256', $this->rawPayload, $this->key, \true));
    }
    /**
    ...
    {
        return \XeroPHP\Helpers::hashEquals($this->getSignature(), $signature);
    }
    /**
     * @return int
     */
