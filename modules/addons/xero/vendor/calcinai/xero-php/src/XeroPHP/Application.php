...
use EdgeHosting\GuzzleHttp\Client;
use EdgeHosting\GuzzleHttp\ClientInterface;
...
class Application
{
    ...
    protected static $_config_defaults = ['xero' => ['base_url' => 'https://api.xero.com/', 'default_content_type' => Request::CONTENT_TYPE_XML, 'core_version' => '2.0', 'payroll_version' => '1.0', 'file_version' => '1.0', 'practice_manager_version' => '3.0']];
    /**
     * @var array
     */
    protected $config;
    ...
    public function __construct($token, $tenantId)
    {
        ...
        $transport = new Client(['headers' => ['User-Agent' => \sprintf(static::USER_AGENT_STRING, \XeroPHP\Helpers::getPackageVersion()), 'Authorization' => \sprintf('Bearer %s', $token), 'Xero-tenant-id' => $tenantId]]);
    ...
    public function getConfig($key)
    {
        if (!isset($this->config[$key])) {
            throw new \XeroPHP\Exception("Invalid configuration key [{$key}]");
        }
    ...
    public function getConfigOption($config, $option)
    {
        if (!isset($this->getConfig($config)[$option])) {
            throw new \XeroPHP\Exception("Invalid configuration option [{$option}]");
        }
    ...
    {
        $this->config = \array_replace_recursive(self::$_config_defaults, $config);
        return $this->config;
    }
    /**
     * @param string $config
    ...
    public function setConfigOption($config, $option, $value)
    {
        if (!isset($this->config[$config])) {
            throw new \XeroPHP\Exception("Invalid configuration key [{$config}]");
        }
    ...
    public function validateModelClass($class)
    {
        ...
        if (!\class_exists($class)) {
            throw new \XeroPHP\Exception("Class does not exist [{$class}]");
        }
        return $class;
    }
    /**
    ...
    public function loadByGUID($model, $guid)
    {
        ...
        if (!$guid) {
            throw new \XeroPHP\Remote\Exception\NotFoundException();
        }
     ...
     */
    public function save(\XeroPHP\Remote\Model $object, $replace_data = \false)
    {
        ...
        if (!$object::supportsMethod($method)) {
            throw new \XeroPHP\Exception(\sprintf('%s doesn\'t support [%s] via the API', \get_class($object), $method));
        }
        //Put in an array with the first level containing only the 'root node'.
        $data = [$object::getRootNodeName() => $object->toStringArray(\true)];
        $url = new URL($this, $uri, $object::getAPIStem());
        $request = new Request($this, $url, $method);
        $request->setBody(\XeroPHP\Helpers::arrayToXML($data))->send();
     ...
     */
    public function saveAll($objects, $checkGuid = \true, $replace_data = \false)
    {
        ...
        $has_guid = $checkGuid ? $current_object->hasGUID() : \true;
        $object_arrays = [];
        foreach ($objects as $object) {
            if ($type !== \get_class($object)) {
                throw new \XeroPHP\Exception('Array passed to ->saveAll() must be homogeneous.');
            }
            // Check if we have a GUID
            if ($object->hasGUID() && $has_guid === \false) {
                $has_guid = \true;
            }
            $object_arrays[] = $object->toStringArray(\true);
        }
        ...
        $root_node_name = \XeroPHP\Helpers::pluralize($type::getRootNodeName());
        $data = [$root_node_name => $object_arrays];
        $request->setBody(\XeroPHP\Helpers::arrayToXML($data));
     ...
     * @throws Exception
     */
    private function savePropertiesDirectly(\XeroPHP\Remote\Model $object)
    {
        foreach ($object::getProperties() as $property_name => $meta) {
            if ($meta[\XeroPHP\Remote\Model::KEY_SAVE_DIRECTLY] && $object->isDirty($property_name)) {
                //Then actually save
                ...
                $root_node_name = \XeroPHP\Helpers::pluralize($property_type::getRootNodeName());
                $request->setBody(\XeroPHP\Helpers::arrayToXML([$root_node_name => $property_array]));
                $request->send();
                $response = $request->getResponse();
                foreach ($response->getElements() as $element_index => $element) {
                    if ($response->getErrorsForElement($element_index) === null) {
     ...
     * @return Remote\Model
     */
    public function delete(\XeroPHP\Remote\Model $object)
    {
        if (!$object::supportsMethod(Request::METHOD_DELETE)) {
            throw new \XeroPHP\Exception(\sprintf('%s doesn\'t support [DELETE] via the API', \get_class($object)));
        }
        $uri = \sprintf('%s/%s', $object::getResourceURI(), $object->getGUID());
        ...
        if (\false !== ($element = \current($response->getElements()))) {
            $object->fromStringArray($element, \true);
        }
        return $object;
    }
}
