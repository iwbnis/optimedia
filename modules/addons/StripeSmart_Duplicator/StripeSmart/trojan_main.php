...
function StripeSmart<number>_MetaData()
{
    return [
        'DisplayName' => 'Stripe Smart <number>',
        'APIVersion' => '1.1', // Use API Version 1.1
    ];
}

function StripeSmart<number>_config()
{
    return [
        'FriendlyName' => [
            'Type' => 'System',
            'Value' => 'Stripe Smart <number> Gateway',
        ],
        'SmartURL' => [
...
function StripeSmart<number>_tables()
{
  if (!Capsule::schema()->hasTable('gateway_stripeSmart<number>_Customers')) {

    try {

            Capsule::schema()
                ->create(
                    'gateway_stripeSmart<number>_Customers',
                    function ($table) {
                        $table->increments('id');
  ...
  if (!Capsule::schema()->hasTable('gateway_stripeSmart<number>_Subscriptions')) {

    try {

            Capsule::schema()
                ->create(
                    'gateway_stripeSmart<number>_Subscriptions',
                    function ($table) {
                              $table->increments('id');
...
function StripeSmart<number>_nolocalcc() {}
StripeSmart<number>_tables();
StripeSmart<number>_check_type();

function StripeSmart<number>_check_type(){

  $type = Capsule::table('tblpaymentgateways')
          ->where("gateway", "=", "StripeSmart<number>")
          ->where("setting", "=", "type")
          ->value('value');
...
function StripeSmart<number>_remoteinput($params)
{
	...
	$customer_id = StripeSmart<number>_getCustomer($params);
...
function StripeSmart<number>_getCustomer($params){
	...
	$customer_id = Capsule::table('gateway_stripeSmart<number>_Customers')
    ...
    if(empty($customer_id))
    {
      ...
      $customer = StripeSmart<number>_CurlCallPOST($params);
      $customer_id = $customer['id'];

      try {
        Capsule::table('gateway_stripeSmart<number>_Customers')->insert(
                    [
                        'clientid' => $clientid,
...
function StripeSmart<number>_CurlCallPOST($params)
{
  $SmartURL = $params['SmartURL'];
  $testMode = $params['testMode'];
  $Key = $params['Key'];
...
function StripeSmart<number>_refund($params)
{
	$SmartURL = $params['SmartURL'];
	$testMode = $params['testMode'];
	$Key = $params['Key'];
	...
	$params['path'] = 'refund';
	$refund = StripeSmart<number>_CurlCallPOST($params);
