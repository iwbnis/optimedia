...
add_hook('ClientAreaProductDetailsOutput', 1, function($service) {
	...
	$gatewayParams = getGatewayVariables('StripeSmart10');

	$serviceId = $service['service']->id;
	
	$subs = Capsule::table('gateway_stripeSmart10_Subscriptions')
	...
	if($_GET['customAction'] == 'cancelSubscription')
	{
		  ...
		  logActivity("StripeSmart10: Subscription Cancel Status: $status -  Action Respose: $json_response");		
		  ...
		  if($response['status'] == 'canceled')
		  {
			       ...
			       try {
          Capsule::table('gateway_stripeSmart10_Subscriptions')
