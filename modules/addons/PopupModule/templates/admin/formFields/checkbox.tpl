...
<div class="col-sm-{$colWidth}" {if $id} id="{$id}" {elseif $addIDs}id="{$addIDs}_{$name}"{/if}>
    {foreach from=$options item=option key=opValue}
        {if !$inline}<div class="checkbox">{/if}
            <label>
                <input type="checkbox" {if is_array($value) && in_array($opValue,$value)}checked="checked"{/if} {if $checked}checked="checked"{/if} name="{$nameAttr}[]" value="{$opValue}" {if $disabled}disabled="disabled"{/if} {foreach from=$dataAttr key=dataKey item=dataValue}data-{$dataKey}="{$dataValue}"{/foreach}>
                {$option}
            </label>
        {if !$inline}</div>{/if}
    {/foreach}
