...
<div class="col-sm-{$colWidth}">
    <div class='input-group date'>
        ...
        <input name="{$nameAttr}" data-timepicker="" type="text" value="{$value}"  class="form-control" {if $addIDs}id="{$addIDs}_{$name}"{/if} placeholder="{if $enablePlaceholder}{$MGLANG->T('placeholder')}{/if}" {foreach from=$dataAttr key=dataKey item=dataValue}data-{$dataKey}="{$dataValue}"{/foreach}>
    </div>
