...
jQuery.fn.MGModalActions = function(){    
                ...
                this.on('click','*[data-modal-id]', function(event){
                    ...
                    var title = jQuery(event.currentTarget).attr('data-modal-title');
                    if(!target)
                    {
                    ...
                    jQuery('#'+modal).find('[data-modal-title]').text(title);
                ...
                this.modalAction = function(action,target,data){
                    ...
                    JSONParser.request(
                        ...
                        , function(data) {
                            ...
                            if(data.deleted && !(data.deleted instanceof Array))
                            {
                                if(typeof that.rowUpdateFunction === 'function')
                                {
                                ...
                                row = null;
                            } else if (data.deleted && (data.deleted instanceof Array)) {
                                jQuery.each(data.deleted,function (i,val) {
                                    if(typeof that.rowUpdateFunction === 'function')
                                    {
                                        val = that.rowUpdateFunction(val);
                                    }
                                    ...
                                    for(var x in val)
                                    {
                                        ...
                                        {
                                            var selector = 'tr';
                                            for(var z in val['DT_RowData'])
                                            {
                                                selector += '[data-'+z+'="'+val['DT_RowData'][z]+'"]';
                                            }
                                            var row = that.find(selector);
                                        }
                                    }
