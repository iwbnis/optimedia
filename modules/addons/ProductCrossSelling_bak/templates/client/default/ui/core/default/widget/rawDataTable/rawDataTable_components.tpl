...
>
    <div class="lu-t-c widgetActionComponent" id="{$elementId}" actionid="{$elementId}" data-table-container data-check-container>
        {if $rawObject->haveInternalAlertMessage()}
            <div class="alert {if $rawObject->getInternalAlertSize() !== ''}alert--{$rawObject->getInternalAlertSize()}{/if} alert-{$rawObject->getInternalAlertMessageType()} alert--faded datatable-alert-top">
        ...
        {/if}
        {if $rawObject->hasFilters()}
            <div class="lu-col-md-12 mg-dt-filters-container">
                {assign var="filtersPerRowCount" value=$rawObject->getFiltersPerRow()}
                {assign var="filtersKeys" value=$rawObject->getFilters()|array_keys}
                {assign var="filtersPerRowCounter" value=1}

                <div class="lu-row mg-filters-wrapper">
                    {foreach from=$rawObject->getFilters() key=filterKey item=filterValue}
                        <div class="lu-col-md-{(12/$filtersPerRowCount)|intval}">
                            {$filterValue->getHtml()}
                        </div>
                    {/foreach}
                </div>
                ...
                <div class="lu-col-md-12">
                    <div class="mg-filter-search-wrapper">
                        <a href="javascript:;" class="lu-btn lu-btn--primary" @click="searchData" :disabled="!searchEnabled">
                            <i class="lu-btn__icon lu-zmdi lu-zmdi-search"></i>
        ...
        <div class="dataTables_wrapper no-footer">
            <div>
                <table class="lu-table lu-table--mob-collapsible dataTable no-footer dtr-column" width="100%" role="grid">
                    ...
                    <tbody>
                    <tr v-for="dataRow in dataRows" {literal}:actionid{/literal}="dataRow.{$rawObject->getActionIdColumnName()}" role="row">
                        {foreach from=$customTplVars.columns key=tplKey item=tplValue}
                            {if $rawObject->hasMassActionButtons() && $collArrKeys[0] === $tplKey}
                                <td>
                                    {if $rawObject->isvSortable()}
