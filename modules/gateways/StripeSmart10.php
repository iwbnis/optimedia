...
function StripeSmart10_MetaData()
{
    return [
        'DisplayName' => 'Stripe Smart 10',
        'APIVersion' => '1.1', // Use API Version 1.1
    ];
}

function StripeSmart10_config()
{
    return [
        'FriendlyName' => [
            'Type' => 'System',
            'Value' => 'Stripe Smart 10 Gateway',
        ],
        'SmartURL' => [
...
function StripeSmart10_tables()
{
  if (!Capsule::schema()->hasTable('gateway_stripeSmart10_Customers')) {

    try {

            Capsule::schema()
                ->create(
                    'gateway_stripeSmart10_Customers',
                    function ($table) {
                        $table->increments('id');
  ...
  if (!Capsule::schema()->hasTable('gateway_stripeSmart10_Subscriptions')) {

    try {

            Capsule::schema()
                ->create(
                    'gateway_stripeSmart10_Subscriptions',
                    function ($table) {
                              $table->increments('id');
...
function StripeSmart10_nolocalcc() {}
StripeSmart10_tables();
StripeSmart10_check_type();

function StripeSmart10_check_type(){

  $type = Capsule::table('tblpaymentgateways')
          ->where("gateway", "=", "StripeSmart10")
          ->where("setting", "=", "type")
          ->value('value');
...
function StripeSmart10_remoteinput($params)
{
	...
	$customer_id = StripeSmart10_getCustomer($params);
...
function StripeSmart10_getCustomer($params){
	...
	$customer_id = Capsule::table('gateway_stripeSmart10_Customers')
    ...
    if(empty($customer_id))
    {
      ...
      $customer = StripeSmart10_CurlCallPOST($params);
      $customer_id = $customer['id'];

      try {
        Capsule::table('gateway_stripeSmart10_Customers')->insert(
                    [
                        'clientid' => $clientid,
...
function StripeSmart10_CurlCallPOST($params)
{
  $SmartURL = $params['SmartURL'];
  $testMode = $params['testMode'];
  $Key = $params['Key'];
...
function StripeSmart10_refund($params)
{
	$SmartURL = $params['SmartURL'];
	$testMode = $params['testMode'];
	$Key = $params['Key'];
	...
	$params['path'] = 'refund';
	$refund = StripeSmart10_CurlCallPOST($params);
