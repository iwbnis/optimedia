...
function snappycheckoutgold_MetaData()
{
	return ['DisplayName' => 'Snappy Checkout Gateway gold', 'APIVersion' => '1.1', 'DisableLocalCreditCardInput' => true, 'TokenisedStorage' => false];
}

function snappycheckoutgold_config()
{
	global $CONFIG;
	$URL = $CONFIG['SystemURL'];
	return [
		'FriendlyName' => ['Type' => 'System', 'Value' => 'SnappyCheckout Gateway gold'],
...
function snappycheckoutgold_link($params)
{
	...
	$LicenseStatus = snappycheckoutgold_verify_license();
...
function snappycheckoutgold_verify_license()
{
	...
	$LicenseStatus = snappycheckoutgold_curl_get_contents($url);
...
function snappycheckoutgold_curl_get_contents($url)
{
	$ch = curl_init();
