...
if (($type == 'checkout.session.completed' && $status == 'paid') || ($type == 'charge.succeeded' && $status == 'succeeded')) {
	...
	if($subId)
	{
		StripeSmart_checkUpdateSubscription($subId , $paramsObject);
	}
}

if($type == 'invoice.paid' && $status == 'paid')
{
	$subscExist = Capsule::table('gateway_stripeSmart_Subscriptions')
	...
	if($subId)
	{
		StripeSmart_checkUpdateSubscription($subId , $params);
	}
...
function StripeSmart_checkUpdateSubscription($subId , $data){
	...
	else
	{
		...
		{
			  try {
					  Capsule::table('gateway_stripeSmart_Subscriptions')->insert(
					  [
