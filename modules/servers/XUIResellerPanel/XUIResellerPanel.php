...
// Check if the field name already exists for the given $pid
$existingField = Capsule::table('tblcustomfields')
    ->where('fieldname', 'Select account') // Modify this to your desired field name
    ->where('relid', $pid)
    ->exists();

if (!$existingField) {
    // Define the INSERT query with column names and placeholders for values
    $insertQuery = "
    ...
    try {
        ...
        $result[] = 'error';
        // You might want to log the error or perform other error handling actions
    }
}
