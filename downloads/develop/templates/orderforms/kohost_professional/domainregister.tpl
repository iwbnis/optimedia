        ...
        <div class="cart-body">
            ...
            <div class="domain-checker-container">
                ...
                <div class="domain-checker-bg clearfix">
                    <form method="post" action="{$WEB_ROOT}/cart.php" id="frmDomainChecker">
                        ...
                        <div class="row">
                            <div class="{if $captcha->isEnabled() && $captcha->isEnabledForForm($captchaForm) && !$captcha->recaptcha->isInvisible()}col-md-7 col-xs-12 col-sm-7{else}col-md-10 col-sm-11 col-xs-11{/if}">
                                <div class="input-group input-group-lg input-group-box">
                                    ...
                                    <span class="input-group-btn input-group-append">
                                        <button type="submit" id="btnCheckAvailability" class="btn search-btn domain-check-availability{$captcha->getButtonClass($captchaForm)}">{$LANG.search}</button>
                                    </span>
                                </div>
                            </div>
            ...
            <div id="DomainSearchResults" class="w-hidden">
                ...
                <div id="searchDomainInfo" class="domain-checker-result-headline">
                    ...
                    <div id="primaryLookupResult" class="domain-lookup-result w-hidden available-block">
                        <p class="domain-invalid domain-checker-invalid"><span class="domainSearch"><i class="far fa-times-circle"></i><br /></span> {lang key='orderForm.domainLetterOrNumber'}<span class="domain-length-restrictions">{lang key='orderForm.domainLengthRequirements'}</span></p>
                        <p class="domain-unavailable domain-checker-unavailable"><span class="domainSearch"><i class="far fa-times-circle"></i><br /></span> {lang key='orderForm.domainIsUnavailable'}</p>
                        <p class="domain-available domain-checker-available"><span class="domainSearch"><i class="far fa-check-circle"></i><br /></span> {$LANG.domainavailable1} <strong></strong> {$LANG.domainavailable2}</p>
                        <a class="domain-contact-support btn primary-solid-btn">{$LANG.domainContactUs}</a>
                        ...
                        <p class="domain-price">
                            ...
                            <button class="btn primary-solid-btn btn-add-to-cart" data-whois="0" data-domain="">
                                <span class="to-add">{$LANG.addtocart}</span>
                                <span class="added"><i class="fas fa-shopping-bag"></i> {lang key='checkout'}</span>
            ...
            <div class="domain-pricing">
                ...
                <div class="tld-pricing-table-wrap">
                    <table class="table vps-hosting-pricing-table vps-hosting-pricing-table-two">
                        <thead>
                        <tr>
                        ...
                        <tr class="vps-pricing-row border-accent-primary tld-row" data-category="{foreach $price.categories as $category}|{$category}|{/foreach}">
                            <td data-value="VCPU">
                                <strong>.{$tld}</strong>
                                {if $price.group}
                            ...
                            <td data-value="Price">
                                {if isset($price.register) && current($price.register) > 0}
                                    <span class="rate color-primary">{current($price.register)}</span>
                                    <span class="pricing-onsale">{key($price.register)} {if key($price.register) > 1}{lang key="orderForm.years"}{else}{lang key="orderForm.year"}{/if}</span>
                                {elseif isset($price.register) && current($price.register) == 0}
                                    <span>{lang key='orderfree'}</span>
                                {else}
                                    <span class="pricing-onsale">{lang key='na'}</span>
                                {/if}
                            </td>
                            <td data-value="Price">
                                 {if isset($price.transfer) && current($price.transfer) > 0}
                                    <span class="rate">{current($price.transfer)}</span>
                                    <span class="pricing-onsale">{key($price.transfer)} {if key($price.register) > 1}{lang key="orderForm.years"}{else}{lang key="orderForm.year"}{/if}</span>
                            ...
                            <td data-value="Price">
                                {if isset($price.renew) && current($price.renew) > 0}
                                    <span class="rate">{current($price.renew)}</span>
                                    <span class="pricing-onsale">{key($price.renew)} {if key($price.register) > 1}{lang key="orderForm.years"}{else}{lang key="orderForm.year"}{/if}</span>
            ...
            <div class="row">
                <div class="{if $domainTransferEnabled}col-md-6{else}col-md-8 col-md-offset-2 offset-md-2{/if}">
                    <div class="domain-promo-box">
                        <div class="clearfix">
                            <i class="fas fa-server fa-3x color-primary"></i>
                            <div>
                                <h5 class="mb-0 no-wrap">{lang key='orderForm.addHosting'}</h5>
                        ...
                        <a href="{$WEB_ROOT}/cart.php" class="btn outline-btn">
                ...
                {if $domainTransferEnabled}
                    <div class="col-md-6">
                        <div class="domain-promo-box">
                            <div class="clearfix">
                                <i class="fas fa-globe fa-3x color-primary"></i>
                                <div>
                                    <h5 class="mb-0">{lang key='orderForm.transferToUs'}</h5>
                                    <p class="font-bold">* {lang key='orderForm.transferExtend'}</p>
                                </div>
                            </div>

                            <p class="small">* {lang key='orderForm.extendExclusions'}</p>
                            <a href="{$WEB_ROOT}/cart.php?a=add&domain=transfer" class="btn outline-btn">
