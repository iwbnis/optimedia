...
{if $unpaidInvoice}
    <div class="alert alert-{if $unpaidInvoiceOverdue}info{else}warning{/if}" id="alert{if $unpaidInvoiceOverdue}Overdue{else}Unpaid{/if}Invoice">
        <div class="pull-right">
            <a href="viewinvoice.php?id={$unpaidInvoice}" class="btn btn-xs pay-invoice">
                {lang key='payInvoice'}
            </a>
        </div>
...
<div class="tab-content margin-bottom">
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <div class="product-status product-status-{$rawstatus|strtolower}">
                            <div class="product-icon text-center">
                                <span class="fa-stack">
                                    <i class="fal fa-{if $type eq "hostingaccount" || $type == "reselleraccount"}hdd{elseif $type eq "server"}database{else}archive{/if} fa-inverse"></i>
                                </span>
                    ...
                    <div class="row mt-20">
                        {if $packagesupgrade}
                            <div class="col-xs-{if $showcancelbutton}6{else}12{/if}">
                                <a href="upgrade.php?type=package&amp;id={$id}" class="btn primary-solid-btn btn-success">{$LANG.upgrade}</a>
                            </div>
                        {/if}
                        {if $showcancelbutton}
                            <div class="col-xs-{if $packagesupgrade}6{else}12{/if}">
                                <a href="clientarea.php?action=cancel&amp;id={$id}" class="btn primary-solid-btn btn-danger {if $pendingcancellation}disabled{/if}">{if $pendingcancellation}{$LANG.cancellationrequested}{else}{$LANG.clientareacancelrequestbutton}{/if}</a>
                            </div>
                        {/if}
                    </div>
                {/if}
                ...
                <div class="row clearfix">
                    <div class="col-xs-12">
                        <ul class="nav nav-tabs nav-tabs-overflow">
                            {if $domain}
                                <li class="active">
                                    <a href="#domain" data-toggle="tab"><i class="fal fa-globe fa-fw"></i> {if $type eq "server"}{$LANG.sslserverinfo}{elseif ($type eq "hostingaccount" || $type eq "reselleraccount") && $serverdata}{$LANG.hostingInfo}{else}{$LANG.clientareahostingdomain}{/if}</a>
                                </li>
                            {elseif $moduleclientarea}
                                <li class="active">
                                    <a href="#manage" data-toggle="tab"><i class="fal fa-globe fa-fw"></i> {$LANG.manage}</a>
                                </li>
                            {/if}
                            ...
                            {if $metricStats}
                                <li{if !$domain && !$moduleclientarea && !$configurableoptions} class="active"{/if}>
                                    <a href="#metrics" data-toggle="tab"><i class="fal fa-chart-line fa-fw"></i> {$LANG.metrics.title}</a>
                                </li>
                            {/if}
                            {if $customfields}
                                <li{if !$domain && !$moduleclientarea && !$metricStats && !$configurableoptions} class="active"{/if}>
                                    <a href="#additionalinfo" data-toggle="tab"><i class="fal fa-info fa-fw"></i> {$LANG.additionalInfo}</a>
                                </li>
                            {/if}
                            {if $lastupdate}
                                <li{if !$domain && !$moduleclientarea && !$configurableoptions && !$customfields} class="active"{/if}>
                                    <a href="#resourceusage" data-toggle="tab"><i class="fal fa-inbox fa-fw"></i> {$LANG.resourceUsage}</a>
                                </li>
                            {/if}
                        </ul>
                    </div>
                    ...
                    {if $domain}
                        <div class="tab-pane fade in active text-center" id="domain">
                            ...
                            {else}
                                ...
                                <br>
                                <p>
                                    <a href="http://{$domain}" class="btn outline-btn" target="_blank">{$LANG.visitwebsite}</a>
                                    {if $domainId}
                                        <a href="clientarea.php?action=domaindetails&id={$domainId}" class="btn outline-btn" target="_blank">{$LANG.managedomain}</a>
                                    {/if}
                                </p>
                            {/if}
                            {if $moduleclientarea}
        ...
        <div class="row">
            {foreach from=$downloads item=download}
                <div class="col-xs-10 col-md-6">
                    <div class="ticket-departments p-5">
                        <h4>{$download.title}</h4>
                        <p>
                            {$download.description}
                        </p>
                        <p class="mb-0">
                            <a href="{$download.link}" class="btn primary-solid-btn"><i class="far fa-download"></i> {$LANG.downloadname}</a>
                        </p>
                    </div>
                </div>
            {/foreach}
    ...
    <div class="tab-pane fade in" id="tabAddons">
        ...
        <h4>{$LANG.clientareahostingaddons}</h4>
    ...
    <div class="tab-pane fade in" id="tabChangepw">
        ...
        <h4>{$LANG.serverchangepassword}</h4>
