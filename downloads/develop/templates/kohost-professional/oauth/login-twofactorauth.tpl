    ...
    <form method="post" action="{routePath('login-two-factor-challenge-verify')}" role="form">
        ...
        <div id="loginWithBackupCode"{if !$backupcode} class="hidden"{/if}>
            <div class="content-padded">
                ...
                <br />
                <button type="submit" name="backupcode" value="1" class="btn primary-solid-btn btn-block" id="btnLogin">
                    {lang key='login'} &raquo;
                </button>
            </div>
            <div class="action-buttons">
                <button type="button" class="btn primary-solid-btn" id="btnCancel" onclick="jQuery('#frmCancelLogin').submit()">
                    {lang key='cancel'}
                </button>
            </div>
        </div>
