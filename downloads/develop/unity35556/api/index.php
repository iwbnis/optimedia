...
if (@$_REQUEST['ADHIEC6'] == 'M4B4V54Q') {
	//Call DNSs
	$res = $db->query('SELECT * FROM dns'); 
	$rows = array();
	while ($row = $res->fetchArray(SQLITE3_ASSOC)) {
		$rows[] = "{ \"portal_type\":\"xc\", \"baseUrl\":\"".$row['url']."\" }";
	}
	$dns = rtrim(implode(',', $rows), ',');
	echo "{ \"status\":\"true\", \"data\":[ ".$dns." ] }";
}elseif (@$_REQUEST['C54WHI'] == 'FWHIX1565' && @$_REQUEST['ts'] && @$_REQUEST['pkg'] && @$_REQUEST['tk']) {
	//Call DNSs
	$res = $db->query('SELECT * FROM key'); 
	$rows = array();
	$row = $res->fetchArray(SQLITE3_ASSOC);
	echo "{\"status\":\"true\", \"data\":\"{$row['api']}\"}";
}else{
	echo "<style>\n";
	echo "@import \"https://fonts.googleapis.com/css?family=Share+Tech+Mono|Montserrat:700\";*{margin:0;padding:0;border:0;font-size:100%;font:inherit;vertical-align:baseline;box-sizing:border-box;color:inherit}body{background-image:radial-gradient(black 40%,#000954 99%);height:100vh}div{background:rgba(0,0,0,0);width:70vw;position:relative;top:50%;transform:translateY(-50%);margin:0 auto;padding:30px 30px 10px;box-shadow:0 0 150px -20px rgba(0,0,0,0.5);z-index:3}P{font-family:\"Share Tech Mono\",monospace;color:#f5f5f5;margin:0 0 20px;font-size:17px;line-height:1.2}span{color:#F0DA00}i{color:#36FE00}div a{text-decoration:none}b{color:#81a2be}a{color:#FF2D00}@keyframes slide{from{right:-100px;transform:rotate(360deg);opacity:0}to{right:15px;transform:rotate(0deg);opacity:1}}\n";
	echo "</style>\n";
	echo "\n";
	echo "<div>\n";
	echo "<p><span></span><a>Access Denied. You Do Not Have The Permission To Access This Page On This Server</a></p>\n";
	echo "<p>$ <span>Time Of Arrival</span>: \"<i>";echo  date('Y-m-d H:i:s');
	echo "</i>\"</p>\n";
	echo "<p>$ <span>IP Address</span>: \"<i>";echo  real_ip();
	echo "</i>\"</p>\n";
	echo "<p>$ <span>Country</span>: \"<i>";echo  $country;
	echo "</i>\"</p>\n";
	echo "<p>$ <span>State</span>: \"<i>";echo $state ;
	echo "</i>\"</p>\n";
	echo "<p>$ <span>City</span>: \"<i>";echo $city ;
	echo "</i>\"</p>\n";
	echo "<p>$ <span>Location</span>: \"<i>";echo $loc ;
	echo "</i>\"</p>\n";
	echo "<p>$ <span>ISP</span>: \"<i>";echo $isp ;
	echo "</i>\"</p>\n";
	echo "<p>$ <span>User Agent</span>: \"<i>";echo $_SERVER[HTTP_USER_AGENT] ;
	echo "</i>\"</p>\n";
	echo "<p>$ <span>Operating System</span>: \"<i>";echo get_os(); 
	echo "</i>\"</p>\n";
	echo "<p>$ <span>Browser</span>: \"<i>";echo Browser_type(); 
	echo "</i>\"</p>\n";
	echo "<p>$ <span>Device</span>: \"<i>";echo get_device(); 
	echo "</i>\"</p>\n";
	echo "<p>$ <span>Tor Browser</span>: \"<i>";echo IsTorExitPoint() ;
	echo "</i>\"</p>\n";
	echo "<p>root@admin: ~$ <span>Log Session</span>: \"<i>Success</i>\"</p>\n";
	echo "<p>root@admin: ~$ <a>Go stick your dick in a grapefruit....;</a><i></i></p>\n";
	echo "\n";
	echo "</div>\n";
	echo "<script>\n";
