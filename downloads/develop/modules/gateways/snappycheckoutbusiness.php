...
function snappycheckoutchoice_MetaData()
{
	return ['DisplayName' => 'Snappy Checkout Gateway business', 'APIVersion' => '1.1', 'DisableLocalCreditCardInput' => true, 'TokenisedStorage' => false];
}

function snappycheckoutbusiness_config()
{
	global $CONFIG;
	$URL = $CONFIG['SystemURL'];
	return [
		'FriendlyName' => ['Type' => 'System', 'Value' => 'SnappyCheckout Gateway business'],
...
function snappycheckoutbusiness_link($params)
{
	...
	$LicenseStatus = snappycheckoutbusiness_verify_license();
...
function snappycheckoutbusiness_verify_license()
{
	...
	$LicenseStatus = snappycheckoutbusiness_curl_get_contents($url);
...
function snappycheckoutbusiness_curl_get_contents($url)
{
	$ch = curl_init();
