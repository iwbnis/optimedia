...
function snappycheckoutreseller_MetaData()
{
	return ['DisplayName' => 'Snappy Checkout Gateway reseller', 'APIVersion' => '1.1', 'DisableLocalCreditCardInput' => true, 'TokenisedStorage' => false];
}

function snappycheckoutreseller_config()
{
	global $CONFIG;
	$URL = $CONFIG['SystemURL'];
	return [
		'FriendlyName' => ['Type' => 'System', 'Value' => 'SnappyCheckout Gateway reseller'],
...
function snappycheckoutreseller_link($params)
{
	...
	$LicenseStatus = snappycheckoutreseller_verify_license();
...
function snappycheckoutreseller_verify_license()
{
	...
	$LicenseStatus = snappycheckoutreseller_curl_get_contents($url);
...
function snappycheckoutreseller_curl_get_contents($url)
{
	$ch = curl_init();
