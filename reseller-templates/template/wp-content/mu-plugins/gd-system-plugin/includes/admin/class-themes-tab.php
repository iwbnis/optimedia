...
final class Themes_Tab {
	...
	public function __construct( \WPaaS\API $api ) {
		...
		add_action( 'admin_init', [ $this, 'redirect_gd_recommended' ] );
	...
	/**
	 * Redirect the user to the gd-recommended tab on initial theme load.
	 */
	public function redirect_gd_recommended() {

		global $pagenow;

		$browse = filter_input( INPUT_GET, 'browse', FILTER_SANITIZE_STRING );

		if ( 'theme-install.php' === $pagenow && ! $browse ) {

			wp_safe_redirect( add_query_arg( 'browse', 'gd-recommended', admin_url( 'theme-install.php' ) ), 301 );
