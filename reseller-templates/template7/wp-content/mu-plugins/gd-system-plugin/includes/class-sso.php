...
final class SSO {
	...
	public function init() {
		...
		if ( ! in_array( $action, [ 'heisenberg', 'sso_login' ], true ) || ! $hash ) {
		...
		// Heisenberg should load the front page into the block editor by default.
		if ( 'heisenberg' === $action ) {

			if ( 'page' === get_option( 'show_on_front' ) && $post_id = (int) get_option( 'page_on_front' ) ) {

				$referrer = ! empty( $_REQUEST['GD_REFERRER'] ) ? $_REQUEST['GD_REFERRER'] : '';

				// Take them to edit the front page.
				...
				// There isn't a front page to edit, go home.
				$redirect = home_url();
		...
		if ( is_int( $user_id ) ) {
			...
			/**
			 * If they come for the Heisenberg experience and somehow they got logged out
			 * we will redirect them to the SSO generator for Heisenberg.
			 */
			if ( 'heisenberg' === $action && GD_ACCOUNT_UID ) {
				...
				wp_redirect(
					esc_url_raw(
						sprintf(
							'https://%s/#/hosting/mwp/v1/site/%s/editor',
	...
	public function login_init() {
		...
		if ( ! $enabled || WP_DEBUG || ! Plugin::is_gd() || ! Plugin::use_simple_ux() || ! defined( 'GD_ACCOUNT_UID' ) || ! GD_ACCOUNT_UID || (bool) filter_input( INPUT_GET, 'wpaas-standard-login' ) || has_action( 'login_form' ) || has_action( 'login_enqueue_scripts' ) ) {
