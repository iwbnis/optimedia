...
class Product_model extends MY_Model{
    ...
    public function get_wishlist_products($data) {
        $where = "";
        ...
        if($where == "") {
            ...
            return $this->db->query("SELECT product_id,product_name,product_slug,product_featured_image FROM product ".$where."")->result_array();
        }
    }
    ...
    public function onlineCount(){
        ...
        foreach ($result as $key => $value) {
            ...
            if($value['type'] == 'user'){
                $data['user'] = array(
                                    'online' => $data['user']['online'] + $value['online'],
                                    'total' => $data['user']['total'] + $value['total'],
                                );
    ...
    function getLanguageHtml($control = 'admincontrol' , $theme = null){
        ...
        if($theme == 'store-default') {
            ...
            $html = '<a href="#" class="js-link text-dark"><img alt="image" src="'. base_url($selected['flag']) .'" class="mr-1" height="15" width="20"/>'. $selected['name'] .'</a><ul class="js-dropdown-list">'.$_html;
            $html .= '</ul>';
        } else {
            $_html = '';
            foreach ($all as $key => $value) {
            ...
            $html = '<a class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img class="selected-language" src="'. base_url($selected['flag']) .'" alt="'. $selected['name'] .'"/><span>'. $selected['name'] .'</span><i class="lni lni-chevron-down"></i>
                    </a><div id="language-dropdown" class="dropdown-menu dropdown-menu-right shadow user-setting country-wrap"><i class="arrow"></i>'. $_html;
            return $html .= '</div>';
        }
        return $html;
    }

    function getCurrencyHtml($control = 'admincontrol', $theme = null){
        ...
        if($theme == 'store-default') {
            $html = '<a href="#" class="js-link1 text-dark" data-currency-symbol="'.$selected['symbol_left'].'">'.$selected['symbol_left'].' '.$selected['title'].' '. $selected['symbol_right'] .'</a><ul class=" js-dropdown-list1">';
            foreach ($all as $key => $value) {
                $html .= '<a class="text-dark" href="'. base_url($control."/change_currency/".$value['code']) .'"><li class="d-flex">'.$value['symbol_left'].' '.$value['title'].' '. $value['symbol_right'] .'</li></a>';
            }
            $html .= '</ul>';
        } else {
            $html = '<a class="nav-link dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" 
                        aria-expanded="false"> 
                        <img src="'.base_url('assets/template/images/dollar-icon.png').'" alt="'.$selected['title'].'" /> 
                        <span>'.$selected['symbol_left'].' '.$selected['title'].' '.$selected['symbol_right'].'</span> 
                        <i class="lni lni-chevron-down"></i>
                    </a>';

            $html .= '<div id="currency-dropdown" class="dropdown-menu dropdown-menu-right shadow user-setting currency-wrap"><i class="arrow"></i>';
            foreach($all as $key => $value)
                $html .= '<a class="dropdown-item" href="'. base_url($control."/change_currency/".$value['code']) .'">
                            '.$value['symbol_left'].' '.$value['title'].' '.$value['symbol_right'].'</a>';
            $html .= '</div>';
        }
        return $html;
    }
    ...
    function getAllProduct($user_id, $user_type, $filter = array()){
        ...
        $query = "SELECT SQL_CALC_FOUND_ROWS
                ...
                (SELECT SUM(amount) FROM wallet WHERE {$clause} type = 'click_commission' AND reference_id = product.product_id AND parent_id=0) as commition_click_count
            FROM product
    ...
    function getnotificationnew($viewfor = null, $user_id,$limit = 0, $filter = array()){
        ...
        $this->db->where('notification_viewfor', $viewfor);
        if($viewfor == 'admin')
            $this->db->where('notification_view_user_id',NUlL);
        $this->db->order_by('notification_id', 'desc');
            ...
            if($shareUser && $shareUser['type'] == 'user' && $product['refer_id'] != $product['vendor_id']){
                if($seller->affiliate_sale_commission_type == 'default'){
                    if($seller_setting->affiliate_sale_commission_type == 'percentage'){
                        ...
                        $data['commission'] = max(($product_price * $affiliate_commission_value),1) / 100;
                    }
            ...
            if($type == 'sale'){
                ...
                if($product['product_commision_type'] == 'default'){
                    ...
                    if($commissionSetting['product_commission_type'] == 'percentage'){
                        ...
                        $commission = max(($product_price * $product_commission),1) / 100;
                    ...
                    $commission = max(($product_price * $product_commision_value),1) / 100;
        ...
        if($type == 'sale'){
            ...
            if($product['product_commision_type'] == 'default'){
                ...
                if($commissionType == 'percentage'){
                    ...
                    $commission = max(($product_price * $sale_commision_value),1) / 100;
    ...
    function setClicks($product_id, $user_id, $viewer_id = 0){
        ...
        //if(($result == 0 && $productsetting['click_allow'] == 'single') || ($productsetting['click_allow'] == 'multiple')){
        if($result == 0){
            $cdate = date('Y-m-d h:i:s');
            $new_record = array(
                'action_type'  => 'click',
