...
class IntegrationModel extends MY_Model{
		...
		if(isset($data['af_id']) && $data['af_id'] && $data['ip']){
				...
				if($orderAlready == 0){
					...
					if(isset($commissionData['vendor_id']) && $commissionData['vendor_id'] != null && $commissionData['vendor_id'] != 0) {
						...
						if(in_array($commissionData['vendor_id'], $data['restricted_vendors'])) {
							die('Integration blocked because minimum deposit not maintained!');
						}	
					}
			...
			if(!empty($transactionsBatch)) {
				...
				if(!empty($notificationBatch)) {
					$this->Product_model->create_batch_data('notification', $notificationBatch);
				}
			}
		} else{
			echo "AINF";
	...
	public function addLogBatch($logs){
		...
		foreach ($logs as $log) {
			$logData[] = array(
				...
				'base_url'       => $log['domain_name'],
				'link'           => $log['link'],
				'click_type'     => $log['click_type'],
				'user_id'        => $log['user_id'],
				'vendor_id'      => isset($log['vendor_id']) ? (int)$log['vendor_id'] : 0,
				'created_at'     => date("Y-m-d H:i:s"),
		...
		if(! empty($logData)) {
			$this->db->insert_batch('integration_clicks_logs', $logData);
		}
	}
    ...
    private function calcCommitions($data, $user_id, $ads_id, $active = false){
    	...
    	if($commissionSetting){
	        ...
	        if($commissionType == 'percentage'){
	            ...
	            $commission = max(($product_price * $comission_sale),1) / 100;
	        }
    ...
    public function getProgramTools($filter = array()){
    	...
    	if (isset($filter['vendor_id']) && !empty($filter['vendor_id'])) {
    		...
    		$query->where("(users.id IS NOT NULL OR integration_tools.vendor_id = 0 )");
    	}
