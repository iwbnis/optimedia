...
class Total_model extends MY_Model{
	public function adminTotals(){
		...
		// $totals['click_action_total'] += (int)$this->db->query('SELECT SUM(counter) as total FROM integration_refer_product_action WHERE is_action=1')->row()->total;
	...
	public function getIntegrationsTotals($integration_filters = false){
		...
		WHERE 
			...
			status > 0 AND 
			comm_from = "ex" AND '. $where3 )->result();
		...
		/*$integration_click_amount_vendor = $this->db->query('SELECT domain_name,SUM(amount) as total FROM `wallet` WHERE is_action=1 AND type="external_click_comm_admin" AND status > 0 AND comm_from = "ex" AND is_vendor=1 AND '. $where3 .' GROUP BY domain_name')->result();
	...
	public function getUserTotals($user_id){
		...
		// Vendor action refer external total
		// $totals['vendor_action_external_total'] += (int) $this->db->query("SELECT SUM(irpa.counter) as total FROM integration_refer_product_action as irpa INNER JOIN integration_tools as it ON it.id = irpa.tools_id WHERE irpa.is_action = 1 AND it.vendor_id = {$user_id}")->row()->total;
	...
	public function getUserBalance($user_id, $filter = []){
		...
		// $sale_localstore_commission = $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE type IN('sale_commission','refer_sale_commission') AND amount > 0 AND status > 0 AND commission_status=0 AND user_id={$user_id} {$wallet_where} ")->row()->total;
		...
		$sale_localstore_commission_query = $this->db->query("SELECT `wallet`.`amount` as commission FROM `order_products` INNER JOIN `order` o ON o.id= `order_products`.`order_id` INNER JOIN `wallet` ON `wallet`.`reference_id`= o.id WHERE `wallet`.`user_id` = {$user_id} AND `wallet`.`comm_from`!='ex' AND `wallet`.`status` > 0 AND `wallet`.`type` IN ('sale_commission','refer_sale_commission') {$wallet_where} group by `wallet`.`id` ")->result();
		...
		// External order sale and refer comission
		...
		$welcome_bonus = $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE type='welcome_bonus' AND user_id={$user_id} {$wallet_where} ")->row()->total;
