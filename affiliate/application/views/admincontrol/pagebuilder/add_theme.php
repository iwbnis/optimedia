			...
			<div class="card m-b-30">
				<div class="card-body">
					...
					<div class="pull-right">
						...
						<button type="button" class="btn btn-primary btn-sm" onclick="$('.toggle-div').slideToggle()"><?= __('admin.edit_setting') ?></button>	
					</div>
					<?php } ?>
							...
							<div class="row">
								<div class="col-sm-8">
									<div class="form-group">
										<label for="example-text-input" class="col-form-label d-block"><b><?= __('admin.theme_name') ?></b></label>
										<input placeholder="<?= __('admin.enter_theme_name') ?>" name="name" value="<?php echo $theme['name']; ?>" class="form-control" required="required" type="text">
									</div>
								</div>
								<div class="col-sm-4">
									<label for="example-text-input" class="col-form-label d-block">&nbsp;</label>
									<button class="btn btn-default btn-success" id="update-product" type="submit"><i class="fa fa-save"></i>  <?= __('admin.save_changes') ?></button>
								</div>
							</div>	
						</div>
					</div>
								...
								<tbody>
									<?php foreach($theme_page as $page){ ?>
										<tr>
											...
											<td class="txt-cntr">
												<a class="btn btn-sm btn-primary" onclick="return confirm(<?= __('admin.are_you_sure_to_edit') ?>);" href="<?php echo base_url();?>pagebuilder/addpage/<?php echo $page['theme_id'];?>/<?= $page['page_id'] ?>"><i class="fa fa-edit cursors" aria-hidden="true"></i></a>
												...
												<a target="_blank" class="btn btn-sm btn-primary"  href="<?php echo base_url();?>pagebuilder/addDesignNew/<?php echo $page['theme_id'];?>/<?= $page['page_id'] ?>"><?= __('admin.design') ?> <i class="fa fa-edit cursors" aria-hidden="true"></i></a>
												<a class="btn btn-sm btn-primary" href="<?= base_url('page/'. $page['slug']) ?>" target='_blank'><?= __('admin.preview') ?></a>
											</td>
										</tr>
									<?php } ?>
								</tbody>
							...
							function fill_order() {
								...
								$( "#tech-companies-1 tbody tr" ).each(function(i,j){
									...
									save_xhr = $.ajax({
										...
										beforeSend:function(){
											$(".loading-state").html("<span class='text-success'>"+'<?= __('admin.saving') ?>'+".. </span>");
										},
										complete:function(){
											$(".loading-state").html("");
										},
