                ...
                <div class="mini-stat clearfix bg-white">
                    <div class="mini-stat-info text-center">
                        <h5 class="counter mt-0 text-primary ">
                            <span data-toggle='tooltip' title="<?= __( 'admin.total_orders' ) ?>" class="no-icon ajax-total_sale_count"><?= (int)$totals['total_sale_count'] ?></span> /
                            <span data-toggle='tooltip' title="<?= __( 'admin.vendor_orders' ) ?>" class="no-icon ajax-vendor_order_count"><?= (int)$totals['vendor_order_count'] ?></span> /
                            <span data-toggle='tooltip' title="<?= __( 'admin.sales' ) ?>" class="no-icon ajax-total_balance"><?php echo $totals['full_total_balance'] ?></span>
...
<div class="col-xl-5">
    <div class="card h-100">
        <li class="list-group-item list-group-item-white font-weight-bold pull-left"><?php echo __('admin.live_logs') ?>
            <div class="pull-right">
                <button class="btn btn-outline-warning btn-sm btn-count-notification" data-toggle="tooltip" title='<?= __('admin.all_notifications') ?>' data-key='live_log' data-type='admin'>
                    <span class="count-notifications">0</span>
                    <i class="fa fa-bell"></i>
                </button>
                <button class="btn btn-blue-grey btn-sm btn-setting" data-toggle="tooltip" title='<?= __('admin.live_log_settings') ?>' data-key='live_log' data-type='admin'>
                        ...
                        <tbody>
                            <?php foreach ($populer_users as $key => $users) { ?>
                                <tr class="<?= $key == 0 ? 'bg-primary text-white' : '' ?>">
                                    ...
                                    <td><?php echo c_format($users['amount']); ?> </br><?php echo __('admin.sales') ?></td>
                                    <td><?php echo c_format($users['all_commition']); ?></br><?php echo __('admin.commissions') ?></td>
                                </tr>
                            <?php } ?>
                        </tbody>
                </table>
        ...
        success:function(json){
            ...
            if(json['serverReq']){
                ...
                if(e != ""){
                    ...
                    $(".server-status").html('<div class="server-error">'+'<?= __('admin.server_error') ?>'+' <i class="fa fa-times-circle-o"></i></div>');
                }
                else {
                    $(".server-errors").html('');
                    $(".server-status").html('<div class="server-ok">'+'<?= __('admin.server_run_ok') ?>'+' <i class="fa fa-check-circle-o"></i></div>');
                }
            }
            var count = 0;
            if(json['live_window']){
