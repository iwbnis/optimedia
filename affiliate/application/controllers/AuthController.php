...
class AuthController extends MY_Controller {
	function __construct() {
		...
		if(!isset($_SESSION['setBrowserLanguage']) || !$_SESSION['setBrowserLanguage']) {
	...
	public function setBrowserLanguage() {
		$_SESSION['setBrowserLanguage'] = true;
	...
	public function vendor_register(){
		...
		if($data['vendor_storestatus']['storestatus'] && ($registration_status['registration_status'] == 1 
	...
	public function verify_registeration($id)
	{
		...
		$verify_user = $this->Common->update('users', $where, $data);
		if($verify_user)
		{
			redirect(base_url().'login');
		}
	...
	public function direct_login()
	{
		...
		$user_details_array = $this->Common->select_where_result('users', $where);
		$this->session->set_userdata(array('administrator'=>$user_details_array));
