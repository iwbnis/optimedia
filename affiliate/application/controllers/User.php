...
class User extends REST_Controller {
    ...
    public function login_post()
    {
        ...
        else
        {
            ...
            $count = $this->Common->get_total_rows('users', $where);
            ...
            {
                $get_user_data = $this->Common->get_data_row('users', $where, $field = '*', 'id');
                ...
                $update_token = $this->Common->update('users', $where, $data);
                if($update_token)
                {
                    $response = array(
                        'status' => TRUE,
        ...
        else
        {
            ...
            if(in_array('plan_details', $requestKeys)) {
                ...
                else if($plan_id != -1) 
                {
                    ...
                    $user_plan = $this->Common->get_data_row('membership_user', $where_plan, 'total_day,expire_at,started_at,status_id,is_active,is_lifetime', 'id');
            ...
            if (in_array('market_tools', $requestKeys)) {
                $data['market_tools'] = $this->generate_market_tools($id, [], 15);
            }
    ...
    public function my_affiliate_links_post()
    {
        ...
        if(isset($verify_data['status']) && $verify_data['status'] == 401) {
            ...
            $json = $this->generate_market_tools($id, $requestFilter);
    ...
    private function generate_market_tools($id, $req_fileter = array(), $limited = false) {
        if($limited !== false) {
            $remain = $limited;
        }
        ...
        $market_tools['form_default_commission'] = $this->Product_model->getSettings('formsetting');

        $market_tools['default_commition'] = $this->Product_model->getSettings('productsetting');
        ...
        $tools = $this->IntegrationModel->getProgramTools($filter);
        ...
        $market_tools['data'] = array_reverse($data_list);
