        ...
        <div class="col-sm-6">
            {function name=outputHomePanels2}
                ...
                {if $item->getName() == 'Active Products/Services'}
                    ...
                    <div menuitemname="SSL Certificates - RapidSSL" class="list-group-item list-group-item-action" id="ClientAreaHomePagePanels-products-services-SSL_Certificates_-_RapidSSL">
                        <strong>#{$newservice->id} - {$newservice->name}</strong> - <a target="_blank" href="clientarea.php?action=productdetails&amp;id={$newservice->id}" class="btn btn-success panel-btn">Manage</a>
                    </div>
                    {/foreach}
                    {foreach $addonservices as $addonservice}
                    <div menuitemname="SSL Certificates - RapidSSL" class="list-group-item list-group-item-action" id="ClientAreaHomePagePanels-products-services-SSL_Certificates_-_RapidSSL">
                        <strong>#{$addonservice->id} - {$addonservice->name}</strong> - <a target="_blank" href="clientarea.php?action=productdetails&amp;id={$addonservice->hostingid}&amp;aid={$addonservice->id}" class="btn btn-success panel-btn">Manage</a>
                    </div>
                    {/foreach}
                </div>
                    {/if}
...
<!---------------------------------Credits-------------------------------------------->
                {if $item->getName() == 'Credit Balance'}
                    <div menuItemName="{$item->getName()}" class="panel custom-panel-box">
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {if $item->getExtra('btn-link') && $item->getExtra('btn-text')}
