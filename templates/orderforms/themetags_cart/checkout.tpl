    ...
    <div class="row">
        ...
        <div class="cart-body">
            <div class="header-lined">
                <h2 class="font-size-22">{$LANG.orderForm.checkout}</h2>
            </div>
            {include file="orderforms/themetags_cart/sidebar-categories-collapsed.tpl"}

            <div class="already-registered d-flex align-items-center justify-content-lg-between">
                <p class="text-sm-left overflow-hidden mb-0">{lang key='orderForm.enterPersonalDetails'}</p>
                <div class="pull-right float-right text-lg-right">
            ...
            <form method="post" action="{$smarty.server.PHP_SELF}?a=checkout" name="orderfrm" id="frmCheckout">
                ...
                <div id="containerExistingUserSignin"{if $loggedin || $custtype neq "existing"} class="w-hidden{/if}">
                    ...
                    {include file="orderforms/themetags_cart/linkedaccounts.tpl" linkContext="checkout-existing"}
                </div>
                ...
                <div id="containerNewUserSignup"{if $custtype === 'existing' || (is_numeric($selectedAccountId) && $selectedAccountId > 0) || ($loggedin && $accounts->count() > 0 && $selectedAccountId !== 'new')} class="w-hidden"{/if}>
                    ...
                    <div{if $loggedin} class="w-hidden"{/if}>
                        {include file="orderforms/themetags_cart/linkedaccounts.tpl" linkContext="checkout-new"}
                    </div>
                ...
                {if $domainsinorder}
                    ...
                    <div class="row margin-bottom mb-5">
                        <div class="col-12">
                ...
                <div class="alert alert-success large-text" role="alert" id="totalDueToday">
                    {$LANG.ordertotalduetoday}: &nbsp; <strong id="totalCartPrice" class="h6 font-weight-bold">{$total}</strong>
                </div>
                ...
                {if !$inExpressCheckout}
                    <div id="paymentGatewaysContainer" class="form-group tt-payment-container">
                        <h6>{$LANG.orderForm.preferredPaymentMethod}</h6>

                        <div class="tt-payment-gateways">
                    ...
                    <div class="cc-input-container{if $selectedgatewaytype neq "CC"} w-hidden{/if}" id="creditCardInputFields">
                        {if $client}
                            <div id="existingCardsContainer" class="existing-cc-grid">
                                {include file="orderforms/themetags_cart/includes/existing-paymethods.tpl"}
                            </div>
                        {/if}
                ...
                <div class="tt-captcha-join-mail">
                    {if $accepttos}
                        <p>
                    ...
                    <button type="submit"
                            id="btnCompleteOrder"
                            class="mt-3 btn btn-primary btn-lg disable-on-click spinner-on-click{if $captcha}{$captcha->getButtonClass($captchaForm)}{/if}"
