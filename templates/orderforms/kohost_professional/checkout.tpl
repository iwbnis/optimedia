    ...
    <div class="row">
        ...
        <div class="cart-body">
            <div class="header-lined">
                <h2 class="font-size-24">{$LANG.orderForm.checkout}</h2>
            </div>
            {include file="orderforms/kohost_professional/sidebar-categories-collapsed.tpl"}

            <div class="already-registered clearfix">
                <div class="pull-right float-right">
                    <button type="button" class="btn secondary-outline-btn{if $loggedin || !$loggedin && $custtype eq "existing"} w-hidden{/if}" id="btnAlreadyRegistered">
                        {$LANG.orderForm.alreadyRegistered}
                    </button>
                    <button type="button" class="btn primary-solid-btn{if $loggedin || $custtype neq "existing"} w-hidden{/if}" id="btnNewUserSignup">
            ...
            <form method="post" action="{$smarty.server.PHP_SELF}?a=checkout" name="orderfrm" id="frmCheckout">
                ...
                {if $custtype neq "new" && $loggedin}
                    <div class="sub-heading">
                        <h3>{lang key='switchAccount.title'}</h3>
                    </div>
                ...
                <div id="containerExistingUserSignin"{if $loggedin || $custtype neq "existing"} class="w-hidden"{/if}>
                    ...
                    <div class="sub-heading">
                        <h3>{$LANG.orderForm.existingCustomerLogin}</h3>
                    </div>
                    ...
                    <div class="custom-card-block mb-40">
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                <button type="button" id="btnExistingLogin" class="btn primary-solid-btn btn-block">
                    ...
                    {include file="orderforms/kohost_professional/linkedaccounts.tpl" linkContext="checkout-existing"}
                </div>
                ...
                <div id="containerNewUserSignup"{if $custtype === 'existing' || (is_numeric($selectedAccountId) && $selectedAccountId > 0) || ($loggedin && $accounts->count() > 0 && $selectedAccountId !== 'new')} class="w-hidden"{/if}>
                    <div {if $loggedin} class="w-hidden"{/if}>
                        {include file="orderforms/kohost_professional/linkedaccounts.tpl" linkContext="checkout-new"}
                    </div>
                    ...
                    {if $customfields}
                        <div class="sub-heading">
                            <h3>{$LANG.orderadditionalrequiredinfo} <br><i><small>{lang key='orderForm.requiredField'}</small></i></h3>
                        </div>
                ...
                {if $domainsinorder}
                    ...
                    <p class="text-muted">{$LANG.orderForm.domainAlternativeContact}</p>
                            ...
                            <div class="row" id="domainRegistrantInputFields">
                                ...
                                <div class="col-sm-12">
                                    <div class="form-group prepend-icon">
                                        ...
                                        <input type="text" name="domaincontacttax_id" id="inputDCTaxId" class="field form-control" placeholder="{lang key=\WHMCS\Billing\Tax\Vat::getLabel()} ({$LANG.orderForm.optional})" value="{$domaincontact.tax_id}">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {if !$inExpressCheckout}
                    <div id="paymentGatewaysContainer" class="form-group">
                        ...
                        <div class="payment-geteways-list">
                    ...
                    <div class="cc-input-container{if $selectedgatewaytype neq "CC"} w-hidden{/if}" id="creditCardInputFields">
                        {if $client}
                            <div id="existingCardsContainer" class="existing-cc-grid">
                                {include file="orderforms/kohost_professional/includes/existing-paymethods.tpl"}
                            </div>
                        {/if}
                                ...
                                <div class="form-group prepend-icon">
                                    <div class="input-group">
                                        ...
                                        <span class="input-group-btn input-group-append">
                                            <button type="button" data-toggle="popover" class="renew-alert" data-placement="bottom" data-content="<img src='{$BASE_PATH_IMG}/ccv.gif' width='210' />">
                        ...
                        <div class="row" id="newCardInfo">
                            <div id="cardNumberContainer" class="col-sm-6 new-card-container">
                                <div class="form-group prepend-icon">
                                    <input type="tel" name="ccnumber" id="inputCardNumber" class="field cc-number-field form-control" placeholder="{$LANG.orderForm.cardNumber}" autocomplete="cc-number" data-message-unsupported="{lang key='paymentMethodsManage.unsupportedCardType'}" data-message-invalid="{lang key='paymentMethodsManage.cardNumberNotValid'}" data-supported-cards="{$supportedCardTypes}" />
                            ...
                            <div class="row form-group new-card-container">
                                ...
                                {if $allowClientsToRemoveCards}
                                    <div id="inputNoStoreContainer" class="col-md-6" style="line-height: 32px;">
                                        ...
                                        <input type="checkbox" class="toggle-switch-success no-icheck  form-control" data-size="mini" checked="checked" name="nostore" id="inputNoStore" value="0" data-on-text="{lang key='yes'}" data-off-text="{lang key='no'}">
                ...
                {if $showMarketingEmailOptIn}
                    ...
                    <div class="marketing-email-optin custom-card-block">
                ...
                <div class="text-left">
                    ...
                    <button type="submit"
                            id="btnCompleteOrder"
                            class="btn primary-solid-btn disable-on-click spinner-on-click{if $captcha}{$captcha->getButtonClass($captchaForm)}{/if}"
