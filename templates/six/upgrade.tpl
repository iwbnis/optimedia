<select id="choose_package" class="form-control" name="package">
	<option class="form-control" value="none" selected>Choose Package</option>
</select>
...
<script>
    jQuery(document).ready(function() {
      jQuery('#choose_package').change(function() {
        var selectedPackage = jQuery(this).find('option:selected').text().trim();
        
        jQuery('#package_table tbody tr').hide(); 
        
        var hasMatch = false;
    
        jQuery('#package_table tbody tr').each(function() {
          var rowText = jQuery(this).find('strong').text().trim();

          if (rowText.indexOf(selectedPackage) !== -1) {
            jQuery(this).show();
            hasMatch = true;
          }
        });
        ...
        if (!hasMatch) {
            jQuery('#upgradeerror').html('No Upgrade Available for this item.');
        } else {
            jQuery('#upgradeerror').html('New Configuration:'); 
        }
      });
});
</script> 